...
void LCD_Draw_Circle(uint8_t x0, uint8_t y0, uint8_t radius, uint16_t color)
{
	...
	while (x >= y)
	{
		LCD_Draw_Point(x0 + x, y0 + y, 1);
		LCD_Draw_Point(x0 + y, y0 + x, 1);
		LCD_Draw_Point(x0 - y, y0 + x, 1);
		LCD_Draw_Point(x0 - x, y0 + y, 1);
		LCD_Draw_Point(x0 - x, y0 - y, 1);
		LCD_Draw_Point(x0 - y, y0 - x, 1);
...
void LCD_Invert_Rect(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1)
{
	for (uint8_t i = x0; i <= x1; i++) {
		for (uint8_t j = y0; j <= y1; j++) {

			LCD_Draw_Point(i,j, !OLED_Get_Point(i, j));
		}
	}
}
